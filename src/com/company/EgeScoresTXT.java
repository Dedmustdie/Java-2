...
public class EgeScoresTXT {

    private Map<String, ArrayList<String>> facultiesScoreMap = new HashMap<>();

    public EgeScoresTXT(String pathOfFacultiesScoreDocument) throws IOException {
        FileInputStream facultiesScore;
        facultiesScore = new FileInputStream(pathOfFacultiesScoreDocument);

        byte[] buff = new byte[facultiesScore.available()];
        facultiesScore.read(buff, 0, buff.length);
        ...
        for (String line : lines) {
            ...
            if (line.replaceAll("\\r", "").matches("Факультет: (([а-яА-Я]+)|(\\s))+")) {
                facultiesScoreMap.put(line.replaceAll("((Факультет: )|(\\r))", ""), ege);
                ege = new ArrayList<>();
            }
    ...
    public void makeScoreFromTxt(String pathOfScoreDocument, String pathToResult) {
        try {
            ...
            for (Map.Entry<String, ArrayList<String>> itemStudent: studentScoreMap.entrySet()) {
                ...
                fileOutput = new FileOutputStream(pathToResult + "\\" + name + "(txttxt).TXT");
    ...
    public void makeScoreFromXml(String pathOfScoreDocument, String pathToResult) {
        try {
            ...
            for (Map.Entry<String, ArrayList<String>> itemStudent: studentScoreMap.entrySet()) {
                ...
                String name = itemStudent.getKey().replaceAll("((ФИО: )|(\\r))", "");
                fileOutput = new FileOutputStream(pathToResult + "\\" + name + "(txtxml).TXT");
                fileOutput.write((name + "\n").getBytes(StandardCharsets.UTF_8));
